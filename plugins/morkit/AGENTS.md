...
1. Đọc file `<MORKIT_ROOT>/commands/<name>.md` nếu tồn tại (default: morkit plugin dir, hoặc `~/.codex/morkit/commands/`)
...
| `${CLAUDE_PLUGIN_ROOT}` | Env var đã được install-codex.sh export vào shell rc; nếu unset, fallback `~/.codex/morkit-source/plugins/morkit` |
...
- **Plugin-bundled hooks không auto-load** trong Codex 0.120.0 (`plugin_hooks` flag chưa tồn tại). Hooks của morkit (`hooks/hooks.json`) cần wire thủ công vào `~/.codex/hooks.json` hoặc `<repo>/.codex/hooks.json` với `codex_hooks = true` trong config.toml
