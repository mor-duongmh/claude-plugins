...
2. Locate the latest report under `docs/morkit/reviews/deep-review-<timestamp>-<target>.md`. If multiple, prefer matching `<target>`.
3. Show the user the first 30 lines of the report and ask: **"Post this report to PR #<n>? (y/N)"**.
4. On `y`, run:
   ```bash
   gh pr comment <n> --body-file docs/morkit/reviews/<file>.md
   ```
